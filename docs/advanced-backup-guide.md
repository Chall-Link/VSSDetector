=================================================
VSSDetector 活用ガイド
復元ポイントスナップショット～ドライブ丸ごと7-Zipアーカイブ

　Chall-Link「VSSDetector」の革命的技術を活用した本格バックアップの実現
　　～世界初のVSS技術活用による企業級バックアップをフリーウェアで～
...
(2) Chall-Link「PreFAS」
※ 体験においては、どちらか片方だけでも問題ありません。
※ PreFAS の操作は、基本 7-Zipや、Zip と同じです。

【 このガイドについて 】
Chall-Link「VSSDetector」で、これまで不可能だった「ある時点で完全にフリーズ保存されたドライブ／フォルダ／ファイル」を一般ユーザーが自由に再利用する事を実現します。

このガイドでは VSSDetectorと PreFAS を使い、Windows復元ポイントを使って「7-Zipでドライブ丸ごとバックアップ」する具体的な手順をご紹介します。
別ソフト Chall-Link「PreFAS」は、7-Zipの機能拡張スクリプト・ソフトウェアです。
また、より簡単に体験いただくために 7-Zip単体での小規模フォルダのバックアップの実例も掲載しています。
（2025年6月7日現在で、PreFASは、VSSDetectorとほぼ同時期に公開予定です。）
...
【 事前準備：復元ポイントの作成 】
※共同利用PC、企業環境では必ず管理者・責任者と事前相談・承認を得てから実施して下さい
※復元ポイント作成には、目安として対象ドライブに20%以上の空き容量が必要になります

■ ステップ1：システムの復元機能の有効化

(1) **システムの復元設定画面へのアクセス**
Windowsの検索窓で「復元ポイントの作成」を検索し開く
→ 「システムのプロパティウィンドウ／システムの保護タブ」が開きます

(2) **システムドライブ（Cドライブ）の保護設定【必須設定】**
システムの復元機能は、Cドライブを常に有効にする事が前提になっています。
- Windows（システム）の保護が無効の場合は、Cドライブを選択し
- 「構成」→「システムの保護を有効にする」を選択して Cドライブの保護を有効にします
- **ディスク領域の使用量：ディスクサイズの5-10%を推奨設定**

(3) **データドライブ（Dドライブなど）の保護設定【重要設定】**
- バックアップ対象のデータドライブを慎重に選択し「構成」をクリック
- 「システムの保護を有効にする」を選択
- **ディスク領域の使用量：1-3%を推奨設定**（小容量ドライブは1-2%）
※有効にしただけではまだ復元ポイントは作成されません

■ ステップ2：復元ポイントの作成

(4) **復元ポイント作成プロセスの開始**
「システムのプロパティウィンドウ／システムの保護タブ」の「作成」ボタンをクリック

(5) **識別用説明の入力**
復元ポイントの説明欄に識別しやすい名称を入力
例：「PreFAS-25-06-06-1200」

(6) **復元ポイント作成処理の実行**
「作成」ボタンをクリック → 有効化されている全ドライブのスナップショットの作成が開始

(7) **作成成功の確認**
「復元ポイントは正常に作成されました」の表示を確認
→ **現時点でのドライブの静的が状態保存＝高品質スナップショットが作成完了**

これでスナップショットに使う、復元ポイントが作成されました！

■ VSSDetectorの革命的なポイント
・**復元ポイント＝「その瞬間のドライブ状態」の完全なスナップショット**
・**作成後に実ファイルを自由に変更しても、スナップショット内容は絶対に変化しない**
・**これにより「絶対的に安定したバックアップ元」と「完全に自由な作業環境」の理想的両立が実現**


【 Chall-Link「VSSDetector」による VSSパス取得 】

■ ステップ1：VSSDetectorの実行
...
■ ステップ2：VSSDetector VSSパス情報の確認と活用
