...
### 1. 目標：VSS スナップショット内のフォルダを、アーカイブソースとして 7-Zipアーカイブを作成  
...
2.1.1 **システムの復元設定画面へのアクセス**
Windowsの検索窓で「復元ポイントの作成」を検索し開く
→ 「システムのプロパティウィンドウ／システムの保護タブ」が開きます
  
2.1.2 **システムドライブ（Cドライブ）の保護設定【必須設定】**
  システムの復元機能は、システムドライブ（Cドライブ）が有効である事が前提になっています
  Windows（システム）の保護が無効の場合は、Cドライブを選択し
  「構成」→「システムの保護を有効にする」を選択して Cドライブの保護を有効にします
  **ディスク領域の使用量：ディスクサイズの5-10%を推奨設定**
  
2.1.3 **データドライブ（Dドライブなど）の保護設定【重要設定】**
  バックアップ対象のデータドライブを選択し「構成」をクリック
  「システムの保護を有効にする」を選択
  **ディスク領域の使用量：1-3%を推奨設定**（小容量ドライブでは1-2%）
...
### 3. Chall-Link「VSSDetector」による VSSパス取得
